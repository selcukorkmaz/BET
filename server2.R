...
summaryResult = as.data.frame(rbindlist(summaryList))
...
currentPDB <- reactive({
  ...
  current = read.table("consistencyScore.txt", header=T, sep="\t")
  ...
  if(input$seqCluster == "90"){
    
    c[,c("pdbId", "bioAssemblyNr90", "stoich90", "symmetry90")]
  }
  
  if(input$seqCluster == "70"){
    
    c[,c("pdbId", "bioAssemblyNr70", "stoich70", "symmetry70")]
  }
  
  if(input$seqCluster == "40"){
    
    c[,c("pdbId", "bioAssemblyNr40", "stoich40", "symmetry40")]
  }
