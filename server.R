shinyServer(function(input, output, session){
  ...
  output$combinedResults <- DT::renderDataTable({
    ...
    datatable(df, escape=FALSE,  rownames=FALSE, class = 'cell-border hover stripe', extensions = c('Buttons'), options = list(dom = 'T<"clear">lfrtip',buttons = c('copy', 'excel', 'pdf'),  columnDefs = list(list(targets = c(7:11), visible = FALSE)))
